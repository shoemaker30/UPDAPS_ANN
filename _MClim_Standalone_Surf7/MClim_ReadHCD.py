...
import numpy as np
from _MClim_Standalone_Surf7.MClim_njitFunc import FreezeThawCycle
...
def ReadHCM(FilePath, AirTempFile, Imonth_start, tdesign, ClimaticModel):
    ...
    #    Tair_C      = (Tair_F - 32) / 1.8
    # else:
    #    Tair_C      = Tair_F
    DailyMeanTair_F = np.mean(Tair_F.reshape(-1, 24), axis=1)  # Calculate the mean air temperature of each day.
    FreezeMeanDays = np.where(DailyMeanTair_F < 32)[0]  # Index of days with negative degree-days
    FreezeIndex = np.sum(DailyMeanTair_F[FreezeMeanDays])  # Freezing-Index (degreeF-days) in all pavement life
    FreezeIndex /= tdesign  # Freezing-Index (degreeF-days)
    # Calculating the Freezing/Thaw cycles.
    FreezeThaw = FreezeThawCycle(Tair_F)
    print(f'\t\tFreeze Index = {FreezeIndex:.2f} Degree F days per year')
...
def ReadClimateData(FilePath):
    try:
        ...
        raise Exception(f' !!!! --> Error in ReadClimateData: {err}')
...
def RepeatData(Mat, YearMax):
    ...
    print('\t\tThe analysis duration is longer than the climatic data avialable. The LAST YEAR is Copyid to the end ' + \
          'of the data till the design period is met (i.e., not mirroring)...')
    print('\t\t********************************')
    ...
    for ii in range(int(RepeatNumber)):
