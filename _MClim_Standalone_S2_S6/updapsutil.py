...
def run_one_json_file(jsondir, fileName, remoteNAPCOMdatadir, JsonListtxt, fieldtolookfor):
    if fnmatch.fnmatch(fileName, '*.json'):
        ...
        if ('tDesign' in jsontxt) and ("tirePressure" in jsontxt) and (fieldtolookfor in jsontxt):
            ...
            # if os.path.isfile(os.path.join(jsondir, FileNameNoExt + '-Python-output.json')):
            #     completestatus = 'complete'
            # elif os.path.isfile(os.path.join(jsondir, FileNameNoExt + '_error.txt')):
            #     completestatus = 'error'
            # else:
            #     completestatus = 'incomplete'

            # txttoadd = f'{fileName}, {completestatus} \n'
...
def Search4JsonFileParallel(outjson_fld, remoteNAPCOMdatadir, fieldtolookfor):
    runparallel = True
    ...
    if runparallel:
        ...
        Parallel(n_jobs=num_cores, verbose=50)(
            delayed(run_one_json_file)(outjson_fld, fileName, remoteNAPCOMdatadir, JsonListtxt, fieldtolookfor)
            for fileName in os.listdir(outjson_fld))
    else:
        # Loop through each HPMS section
        for fileName in os.listdir(outjson_fld):
            run_one_json_file(outjson_fld, fileName, remoteNAPCOMdatadir, JsonListtxt, fieldtolookfor)
...
def Search4JsonFiles(path, SubDir, JsonList):
    ...
    for fileName in os.listdir(path):
        ...
        if fnmatch.fnmatch(fileName, '*.json'):
            ...
            with open(jsonpath, 'r') as file:
                jsontxt = json.load(file)
                if ('tDesign' in jsontxt) and ("tirePressure" in jsontxt):
...
def create_json_clusters_and_job_files(JsonFiles2Run, numclusters, eltimecoeff, json_folder,
                                       servernode, RemoteCodeDir, RemoteCodeName, ppnreq, anl_user):
    ...
    if numjsonpercluster < ppnreq:
        numjsonpercluster = ppnreq

    allclusters = {}
    n = 0
    ClusterNo = 0

    fldout_job = json_folder + '/io/'
    if not os.path.isdir(fldout_job):
        print(f'--> Making directory: {fldout_job}')
        os.mkdir(fldout_job)
    errorjson = []
    for i in range(numjson):
        ...
        if 'layerTH' in loadedjson:
            layerTH = loadedjson['layerTH']
            THtot_in = np.sum(layerTH)

            EstimatedRunTime_min = eltimecoeff[0] * THtot_in + eltimecoeff[1] * math.exp(eltimecoeff[2] * THtot_in)
            print(f'--> EstimatedRunTime_min: {EstimatedRunTime_min:.4} for {JsonFiles2Run[i]["LocalPath"]}')
            JsonFiles2Run[i]['EstimatedRunTime_min'] = np.ceil(EstimatedRunTime_min)
            JsonFiles2Run[i]['RemotePath'] = JsonFiles2Run[i]['LocalPath']

            if n >= numjsonpercluster or i == numjson:

                # Save the cluster in the folder
                RunTime_hr = math.ceil(RunTime_Min / ppnreq / 60.0 * 15)
                ...
                cluster_pkl_path, cluster_job_path = save_jsoncluster(cluster_jsons, fldout_job,
                                                                      ...
                                                                      RemoteCodeDir, RemoteCodeName, Twall, ppnreq)

                clustercur = {'cluster_jsons': cluster_jsons, 'cluster_key': cluster_key,
                              'cluster_pkl_path': cluster_pkl_path, 'cluster_job_path': cluster_job_path}
                ...
                RunTime_Min = EstimatedRunTime_min
            ...
            print(f'--> Could not add: {JsonFiles2Run[i]["LocalPath"]}')
            errorjson.append(JsonFiles2Run[i]['LocalPath'])

    savejson(fldout_job, 'ErrorJSONs.json', errorjson)

    return allclusters


def save_jsoncluster(JsonList, fldout_job, CurrentKey, ClusterNo, HPCCnode, RemoteCodeDir, RemoteCodeName, Twall, ppnreq):
    ...
    create_job_files(RemoteJobPath, HPCCnode, RemoteCodeDir, RemoteCodeName, RemotePklPath, Twall, ppnreq)
...
def create_job_files(LocalJobPath, HPCCnode, RemoteCodeDir, RemoteCodeName, RemotePklPath, Twall, ppnreq):
    ...
    with open(LocalJobPath, 'w') as outfile:
        ...
        outfile.write('\n')
        outfile.write(f'module load python/3.8.11;\n '
                      f'cd "{RemoteCodeDir}";\n'
...
def f_process_traf_N(traf_N, freq_Ncorr_CR, zi_AC_bu_fat, zi_AC_td_fat, zi_sub_inAC, zi_sub_inBASE, zi_sub_inSUBG,
    ...
    traf_NCor = {}
    traf_NCor['NAxles'] = traf_N['NAxles']
...
def f_process_calib_coeffs(inps, Va_PV_inAC, Vbeff_PV_inAC, TH_subAC, TH_subBASE, zi_sub_inAC):
    ...
    #     kriz[i] = kri
    #     Briz[i] = Bri
    kriz = np.zeros((len(zi_sub_inAC), 3))
    Briz = np.zeros((len(zi_sub_inAC), 3))
    for i in range(len(zi_sub_inAC)):
        kriz[i] = kri
        Briz[i] = Bri
    # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
def loadjson(localjsonfld, jsonfile):
    with open(os.path.join(localjsonfld, jsonfile), 'r') as f:
        data = json.load(f)
    return data


def savejson(localjsonfld, jsonfile, data):
    with open(os.path.join(localjsonfld + jsonfile), 'w') as outfile:
        json.dump(data, outfile)


def appendtxt(txtfld, txtfile, txttoadd):
    with open(os.path.join(txtfld, txtfile), 'a') as outfile:
